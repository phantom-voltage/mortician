...
#    line2 = "Killed by %s %s\n" % (p['how'], dlvl)
    ...
    conn.execute('''INSERT INTO pcb_scores (name, pts, sex, raceid, classid, pers, clvl, max_clvl, dlvl, max_dlvl, gold, turns, winner, how)
                                values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);''',
                (str(p['name']), int(p['pts']), str(p['sex']), int(p['raceid']), int(p['classid']), int(p['pers']), int(p['clvl']), int(p['max_clvl']), int(p['dlvl']), int(p['max_dlvl']), int(p['gold']), int(p['turns']), int(0), str(p['how'])))
...
#    path = sys.argv[1]
    ...
    var_file = self.var_dir+"/.pcb_players"
